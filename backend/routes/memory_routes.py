...
async def refresh_short_term_memory() -> dict:
    print("[MemoryRoutes] Запрос на обновление краткосрочной памяти через API.")
    ensure_short_term_schema()

    char_name = get_config_value("char_name")
    if not char_name:
    ...
    log_audit_entry(
        ...
        "[MemoryRoutes] Выполнено обновление краткосрочной памяти.",
        AuditStatus.INFO,
...
async def list_short_term_memory() -> dict:
    print("[MemoryRoutes] Получение записей краткосрочной памяти через API.")
    ensure_short_term_schema()
    records = load_recent_records()
    payload = []
    ...
    log_audit_entry(
        "memory_route_list",
        "[MemoryRoutes] Возвращаем список записей краткосрочной памяти.",
        AuditStatus.INFO,
